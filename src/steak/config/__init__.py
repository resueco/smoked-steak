...
APPNAME = "smoked-steak"
DEFAULT_CONFIG_URL = os.environ.get("SMOKED_STEAK_DEFAULT_CONFIG_URL")
...
def get_default_config_path() -> Path:
    ...
    if not default_config_path.exists():
        ...
        if not DEFAULT_CONFIG_URL:
            raise FileNotFoundError(
                "Default config file is missing and SMOKED_STEAK_DEFAULT_CONFIG_URL is not configured."
            )

        click.secho("Downloading default config...", fg="blue")

        default_config_path.parent.mkdir(parents=True, exist_ok=True)

        try:
            response = requests.get(DEFAULT_CONFIG_URL, timeout=30)
...
def setup_config() -> Cfg:
    try:
        ...
        user_choice = click.confirm(f"Do you want smoked-steak to create a default config file at {cfg_path}?")
        if user_choice:
            try:
        ...
        click.secho(
            ...
            "or re-run smoked-steak to generate one from the default template.",
            fg="yellow",
        )
        sys.exit(1)
