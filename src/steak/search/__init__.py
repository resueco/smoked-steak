...
def get_search_sources(sources: dict[str, Any] | None = None) -> dict[str, Any]:
    """Return metadata search sources after applying config-level disables."""
    selected_sources = (
        SEARCHSOURCES
        if sources is None
        else {name: source for name, source in SEARCHSOURCES.items() if name in sources}
    )
...
async def metas(searchstr: tuple[str, ...], track_count: int | None, limit: int) -> None:
    ...
    results = await run_metasearch([search_query], limit=limit, track_count=track_count, sources=search_sources)
    not_found: list[str] = []
    inactive_sources: list[str] = []
    source_errors = set(search_sources.keys()) - set(results)
...
async def run_metasearch(
    ...
    """
    sources = get_search_sources(sources)
