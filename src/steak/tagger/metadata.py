...
from steak.common import handle_scrape_errors, make_searchstrs, re_strip
from steak.search import SEARCHSOURCES, get_search_sources, run_metasearch
from steak.tagger.combine import combine_metadatas
from steak.tagger.sources import METASOURCES, get_metadata_sources, is_metadata_source_disabled
from steak.tagger.sources.base import generate_artists


def _is_source_disabled(source_name: str) -> bool:
    return is_metadata_source_disabled(source_name)


def _enabled_search_sources() -> dict[str, Any]:
    return get_search_sources()


def _enabled_metadata_sources() -> dict[str, Any]:
...
async def get_metadata(path: str, tags: dict[str, Any], rls_data: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    ...
    search_sources = _enabled_search_sources()
    metadata_sources = _enabled_metadata_sources()
    click.echo(f"Search services: {', '.join(search_sources) or 'none'}")
    click.echo(f"Metadata URL services: {', '.join(metadata_sources) or 'none'}")
    if not rls_data:
...
def _print_search_results(results, rls_data=None):
    ...
    for source, releases in results.items():
        ...
        if releases:
            ...
            limited_results = dict(islice(releases.items(), cfg.upload.search.limit))
            for rls_id, release in limited_results.items():
    ...
    while True:
        ...
        for r in res.split():
            ...
            # Handle numeric choices
                ...
                scraper = metadata_sources[choices[int(stripped)][0]].Scraper()
