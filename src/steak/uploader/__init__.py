...
from steak.converter.transcoding import (
    generate_transcode_description,
    transcode_folder,
)
from steak.errors import AbortAndDeleteFolder, CRCMismatchError, EditedLogError, InvalidMetadataError, RequestError
from steak.images import upload_cover
from steak.tagger import (
    metadata_validator_base,
    validate_encoding,
    validate_source,
)
from steak.tagger.ai_review import review_metadata_with_ai
from steak.tagger.audio_info import (
    check_hybrid,
    gather_audio_info,
    recompress_path,
)
...
if TYPE_CHECKING:
    from steak.tagger.tagfile import TagFile
...
@click.option(
    "--tracker",
    "-t",
    callback=steak.trackers.validate_tracker,
...
async def up(
    ...
    if request:
        request = steak.trackers.validate_request(gazelle_site, request)
...
async def upload(
    ...
    remaining_gazelle_sites = list(steak.trackers.tracker_list)
