...
@click.option(
    "--tracker",
    "-t",
    type=click.Choice(steak.trackers.tracker_list, case_sensitive=False),
    help=f"Choices: ({'/'.join(steak.trackers.tracker_list)})",
)
@click.option(
    "--metadata",
    "-m",
...
async def checkconf(tracker: str | None, metadata: bool, seedbox: bool, reset: bool) -> None:
    ...
    if not (metadata or seedbox) or tracker:
        trackers = [tracker] if tracker else steak.trackers.tracker_list

        for t in trackers:
            click.secho(f"\n[ Testing Tracker: {t} ]", fg="cyan", bold=True)
            failed_checks: list[str] = []

            tracker_instance = steak.trackers.get_class(t)()
...
async def health() -> None:
    """Check the status of smoked-steak's config files and command line dependencies."""
    try:
        config_path = find_config_path()
